...
clearly-distinct documentation sets) but won't capture deep semantic paraphrase
...
In this multi-tenant project the embedder is fit PER TENANT: each tenant gets
its own vocabulary and its own SVD basis, so a tenant's vectors only ever live
in a space built from that tenant's own documents. This is a small but real
part of what makes tenant isolation structural rather than a filter (see
vectorstore.py and tenant_gateway.py).
...
Everything downstream (vectorstore.py, tenant_gateway.py, rag_chain.py) talks to
this class only through the standard `embed_documents`/`embed_query` interface,
so swapping the implementation requires no other code changes. That interface
contract IS the actual lesson here — production RAG systems are built so the
...
class LocalTfidfEmbeddings:
    ...
    def __init__(self, n_components: int = 128, random_state: int = 42):
        ...
        self.vectorizer = TfidfVectorizer(
            lowercase=True,
            stop_words="english",
            ngram_range=(1, 2),     # unigrams + bigrams capture phrases like "parental leave"
            max_df=0.9,
            min_df=1,
        )
